...
fLocal_2={
ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(obScriptParam_0, 0f, 0f, 0f) 
};
...
if(!ENTITY::IS_ENTITY_DEAD(pedLocal_1, false))if(!ENTITY::IS_ENTITY_ON_SCREEN(pedLocal_1)) PED::DELETE_PED(&pedLocal_1);
else PED::SET_PED_KEEP_TASK(pedLocal_1, true);
SCRIPT::TERMINATE_THIS_THREAD();
return;
}
...
if(!PED::IS_PED_INJURED(pedLocal_1)){
if(ENTITY::IS_ENTITY_AT_ENTITY(PLAYER::PLAYER_PED_ID(), pedLocal_1, vector, false, true, 0)) return true;
if(ENTITY::HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(pedLocal_1, PLAYER::PLAYER_PED_ID(), true)) return true;
if(ENTITY::IS_ENTITY_TOUCHING_ENTITY(PLAYER::PLAYER_PED_ID(), pedLocal_1)) return true;
}
...
}elseif(ENTITY::HAS_ENTITY_ANIM_FINISHED(pedLocal_1, sLocal_8, sLocal_6, 3)){
TASK::TASK_PLAY_ANIM(pedLocal_1, sLocal_8, sLocal_6, 1090519040, -1056964608, -1, 0, 0, false, false, false);
}elseif(!bLocal_9){
