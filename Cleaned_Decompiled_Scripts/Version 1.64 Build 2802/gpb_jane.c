...
if(func_107()) func_114(0);
if(SYSTEM::VMAG2(ENTITY::GET_ENTITY_VELOCITY(PLAYER::PLAYER_PED_ID())) > 1369f && !func_106()) func_114(0);
if(MISC::GET_GAME_TIMER() < Global_32428 + 10000 && !func_106()) func_114(0);
...
if(!_DOES_ENTITY_EXIST_AND_IS_ALIVE(pedLocal_240)){
if(!bLocal_441) func_97(func_113(SCRIPT::GET_THIS_SCRIPT_NAME()));
func_114(0);
}
...
if(ENTITY::IS_ENTITY_OCCLUDED(pedLocal_240) && func_84(pedLocal_240, PLAYER::PLAYER_PED_ID(), true) > 50f && !BRAIN::IS_WORLD_POINT_WITHIN_BRAIN_ACTIVATION_RANGE()) func_114(1);
SYSTEM::WAIT(0);
}
return;
}
...
if(!ENTITY::IS_ENTITY_DEAD(pedLocal_240, false)) PED::SET_PED_MOVEMENT_CLIPSET(pedLocal_240, sLocal_448, 1048576000);
return;
}
...
if(character==CHAR_TREVOR) return 6;
...
eCharacter character;
character=_GET_CURRENT_PLAYER_CHARACTER();
if(character==CHAR_TREVOR) TEXT_LABEL_ASSIGN_STRING(sParam0, "ImAnActualized", 64);
elseif(character==CHAR_MICHAEL) TEXT_LABEL_ASSIGN_STRING(sParam0, "IAmWellonTheWay", 64);
elseif(character==CHAR_FRANKLIN) TEXT_LABEL_ASSIGN_STRING(sParam0, "EverythingIsUp", 64);
return;
}
...
var func__83(BOOL bParam0, var uParam1, var uParam2) // Position - 0x2D50{
...
Vector3 func__87(float fParam0, var uParam1, var uParam2) // Position - 0x2E77{
float num;
float num2;
...
iLocal_449=1;
if(!ENTITY::IS_ENTITY_DEAD(pedLocal_240, false)) PED::SET_PED_PROP_INDEX(pedLocal_240, 1, 0, 0, false, 1);
return;
}
...
, 4);
if(MISC::ARE_STRINGS_EQUAL(&string1, &uLocal_500)) func_13();
...
if(iLocal_449==1)if(STREAMING::HAS_ANIM_DICT_LOADED(sLocal_448)) STREAMING::REMOVE_ANIM_DICT(sLocal_448);
return;
}
